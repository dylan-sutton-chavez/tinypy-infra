...
impl SSAChunk {
    ...
    fn push_const(&mut self, v: Value) -> u16 {
        if self.constants.len() >= u16::MAX as usize {
            return 0;
        } // A04:2021 – Insecure Design: prevent u16 index overflow on constant pool.
    ...
    fn push_name(&mut self, n: &str) -> u16 {
        ...
        if self.names.len() >= u16::MAX as usize {
            return 0;
        } // A04:2021 – Insecure Design: prevent u16 index overflow on name pool.
...
impl<'src, I: Iterator<Item = Token>> Parser<'src, I> {
    ...
    fn mid_block(&mut self) {
        let Some(j) = self.join_stack.last_mut() else {
            return;
        };
    ...
    fn commit_block(&mut self) {
        let Some(j) = self.join_stack.pop() else {
            return;
        };
    ...
    fn peek(&mut self) -> Option<TokenType> {
        loop {
            match self.tokens.peek().map(|t| t.kind) {
                ...
                Some(TokenType::Nl | TokenType::Comment) => {
                    self.tokens.next();
                }
        ...
        match self.peek() {
            ...
            Some(TokenType::Async) => {
                ...
                match self.peek() {
                    ...
                    Some(TokenType::For) => {
                        self.for_stmt_inner(true);
                    }
                    Some(TokenType::With) => {
                        self.with_stmt_inner(true);
                    }
                    _ => {}
                }
                false
            ...
            Some(TokenType::Del) => {
                ...
                let idx = self
                    .chunk
                    .push_name(&Self::ssa_name(&name, self.current_version(&name)));
            ...
            Some(TokenType::Comma) => {
                ...
                if matches!(self.peek(), Some(TokenType::Equal)) {
                    ...
                    if let Some(sp) = star_pos {
                        ...
                        self.chunk
                            .emit(OpCode::UnpackSequence, targets.len() as u16);
                    }
                    for target in targets.into_iter().rev() {
                        self.store_name(target.trim_start_matches('*').to_string());
                    }
...
impl<'src, I: Iterator<Item = Token>> Parser<'src, I> {
    ...
    fn infix_bp(&mut self, min_bp: u8) {
        loop {
            ...
            let Some((l_bp, r_bp, op)) = Self::binding_power(&tok) else {
                break;
            };
            if l_bp < min_bp {
                break;
